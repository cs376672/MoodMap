...
export const REAL_SPOTS: Record<string, SpotData[]> = {
  ...
  'JP-TYO': [
    ...
    { name: '신주쿠 교엔', lat: 35.6852, lng: 139.7100, desc: '도심 속 거대한 정원. 벚꽃과 울창한 숲 속에서 평화로운 힐링을 선사합니다.', keywords: ['nature', 'healing'] },
  ...
  'JP-OKA': [
    ...
    { name: '만자모', lat: 26.5049, lng: 127.8502, desc: '코끼리 모양의 기암괴석과 에메랄드빛 바다가 만들어내는 탁 트인 자연 절경.', keywords: ['nature', 'healing'] },
...
export const getGenericSpots = (regionName: string, baseLat: number, baseLng: number): SpotData[] => {
  ...
  // 템플릿을 기반으로 300개의 스팟 생성 (중복 방지 및 100% 온테마 폴백용 넉넉한 수량)
  for (let i = 0; i < 300; i++) {
    const template = templates[i % templates.length];
    
    // 중심가 주변 반경 10~20km 내외로 퍼트림
